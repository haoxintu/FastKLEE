...
// THX
#include <set>
extern std::string recording_file;
std::set<std::string> recordingSet;
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Load: {
    ...
    //llvm::errs() << *ki->inst  << "\n";
    //llvm::errs() << ki->inst->getFunction()->getName() << "\n";
    //llvm::errs() << "### Filename : " << ki->info->file << "\n";
    //llvm::errs() << "### Line : " << ki->info->line << "\n";
    //llvm::errs() << "### Column : " << ki->info->column << "\n";
    //llvm::errs() << recording_file << "\n";
    std::string record_klee = ki->info->file + ki->inst->getFunction()->getName().str() +
        std::to_string(ki->info->line) + std::to_string(ki->info->column);
    /*
    //llvm::errs() << "### record in klee : " << record_klee << "\n";
    if (recordingSet.count(record_klee)) { // the element is in the set, count is 1
        llvm::errs() << "Found the records! checking bound! in load" << "\n";
        executeMemoryOperationForCcured(state, false, base, 0, ki);
        break;
        //exit(1);
    }
    else {
  ...
  case Instruction::Store: {
    ...
    //llvm::errs() << "### record in klee : " << record_klee << "\n";
    if (recordingSet.count(record_klee)) { // the element is in the set, count is 1
        llvm::errs() << "Found the records! checking bound in store" << "\n";
        executeMemoryOperationForCcured(state, true, base, value, 0);
        break;
        //exit(1);
    }
    else {
    */
        ...
        executeMemoryOperation(state, true, base, value, ki); // passing ki into the function
        break;
    //}
  }
...
void Executor::executeMemoryOperationForCcured(ExecutionState &state,
                                      bool isWrite,
                                      ref<Expr> address,
  ...
  if (success) {
    ...
    //printf("address = %p\n", toConstant(state, address, "address")->getZExtValue());
    ref<Expr> offset = mo->getOffsetExpr(address);
    //offset->dump();
    // Pre-checking testing
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (success) {
    ...
    //printf("address = %p\n", toConstant(state, address, "address")->getZExtValue());
    ref<Expr> offset = mo->getOffsetExpr(address);
    ref<ConstantExpr> o = toConstant(state, offset, "11");
    int off = o->getZExtValue();
    bool inBounds;
    int a = recordingSet.count(record_klee);
    if (a == 1) { // the element is in the set, count is 1
        ref<Expr> check = mo->getBoundsCheckOffset(offset, bytes);
        check = optimizer.optimizeExpr(check, true);
        solver->setTimeout(coreSolverTimeout);
        bool success = solver->mustBeTrue(state, check, inBounds);
...
void Executor::runFunctionAsMain(Function *f,
  ...
  // THX read recordings from .txt and store them to recordingSet
    std::ifstream file_name;
    std::string temp;
    std::string filename = "/home/haoxin/disk-smu/research/datasets/coreutils-9.0/obj-llvm-ccured/src/" + recording_file.substr(2) + ".txt";
    //llvm::errs() << "name : " << filename << "\n";
    file_name.open(filename);
    if (file_name.fail()){
        llvm::errs() << "Error when opening recordings.txt file!" << "\n";
        return;
    }
    while (std::getline(file_name, temp)){
        //std::istringstream iss(temp);
        // cout << "//" << temp << endl;
        recordingSet.insert(temp);
    }
    file_name.close();
    //for (std::set<std::string>::iterator it=recordingSet.begin(); it!=recordingSet.end(); ++it)
    //    llvm::errs() << *it << "\n";
    //llvm::errs() << "size of recordingSet = " << recordingSet.size() << "\n";
    /// THX end
