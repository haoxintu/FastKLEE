...
/*
std::map<std::string, std::map<int, std::string>> storeOrigMap;
std::map<std::string, std::map<int, std::string>> loadOrigMap;
std::map<std::string, std::string> instOrigMap;

std::map<std::string, std::map<int, std::vector<std::string>>> storeInstMap;
std::map<std::string, std::map<int, std::vector<std::string>>> loadInstMap;
std::map<std::string, std::vector<std::string>> instMap;
int indexx = 1;
*/
/*
...
namespace
{
	...
	public:
		...
		bool processInstruction(Instruction *I)
		{
			...
			else if (StoreInst *II = dyn_cast_or_null<StoreInst>(I))
			{
				...
				if (valoperand->getType()->isPointerTy())
				{
					...
					if (!differentBasicBlock)
					{
						if (varinfo != NULL)
						{
                            ...
                            errs() << "This is the information for store instruction \t" << "\n";
                            errs() << "     " << PtrTypeToString(varinfo->classification) << "\n";
                            errs() << "     " << *II << "\n";
                            errs() << "     " << II->getFunction()->getName() << "\n";
                            const DebugLoc &location = II->getDebugLoc();
                            record_filename = location->getFilename().str();
                            errs() << " ### source code filename : " << location->getFilename() << "\n";
                            errs() << " ### source code line : " << location.getLine() << "\n";
                            errs() << " ### source code column : " << location->getColumn() << "\n";
                            std::string record = location->getFilename().str() + II->getFunction()->getName().str()
                                + std::to_string(location.getLine()) + std::to_string(location->getColumn());
                            errs() << " **** record : " << record << "\n";
                            std::string type = PtrTypeToString(varinfo->classification);
                            if (type == "SEQ" || type == "DYN"){
                                errs() << "marking" << "\n";
                                records.insert(record);
                            }
                            /*
                            std::string func_name = II->getFunction()->getName();
                            std::string key = func_name + std::to_string(indexx);
                            std::vector<std::string> value;
                            std::string str;
                            llvm::raw_string_ostream ss(str);
                            ss << *II;
                            std::string inst = ss.str();
                            value.push_back(inst);
                            value.push_back(PtrTypeToString(varinfo->classification));
                            instMap.insert(std::pair<std::string, std::vector<std::string>>(key, value));
                            indexx ++;
                            */
                        ...
                        /*
                            ...
                            // new added for no found address
                            //const DebugLoc &location = II->getDebugLoc();
                            //errs() << " source code line : " << location.getLine() << "\n";
                            std::string func_name = II->getFunction()->getName();
                            std::string key = func_name + std::to_string(indexx);
                    ...
                    /*
                            ...
                            // new added for from different basic block
                            //const DebugLoc &location = II->getDebugLoc();
                            //errs() << " source code line : " << location.getLine() << "\n";
                            std::string func_name = II->getFunction()->getName();
                            std::string key = func_name + std::to_string(indexx);
                ...
                /*
                    ...
                    //new added for normal addresses
                            //const DebugLoc &location = II->getDebugLoc();
                            //errs() << " source code line : " << location.getLine() << "\n";
                            std::string func_name = II->getFunction()->getName();
                            std::string key = func_name + std::to_string(indexx);
			...
			else if (LoadInst *II = dyn_cast_or_null<LoadInst>(I))
			{
				...
				if (II->getType()->isPointerTy())
				{
					...
					if (varinfo != NULL)
					{
						...
						errs() << PtrTypeToString(varinfo->classification) << " \n";
						TheState.ClassifyPointerVariable(II, varinfo->classification);
                        errs() << "This is the information for load instruction \t" << "\n";
                        errs() << "     " << PtrTypeToString(varinfo->classification) << "\n";
                        errs() << "     " << *II << "\n";
                        errs() << "     " << II->getFunction()->getName() << "\n";
                        std::string func_name = II->getFunction()->getName();
			...
			else if (CastInst *II = dyn_cast_or_null<CastInst>(I))
			{
				...
				if (srcT->isPointerTy())
				{
					...
					if (countIndirections(srcT) != countIndirections(dstT) || innerSrcT->isIntegerTy() != innerDstT->isIntegerTy())
					{
						...
						/*
						for (Type::subtype_iterator STI = innerSrcT->subtype_begin(), STE = innerSrcT->subtype_end(); STI != STE; ++STI){
						if((llvm::Type*)STI == innerDstT){
							errs() << "Downcast: " << *II << " From: " << *srcT << " To: " << *dstT << "\n";
						}
						else
							errs() << "No Downcast" << *II << " From: " << *srcT << " To: " << *dstT << "\n";
						}
						for (Type::subtype_iterator DTI = innerDstT->subtype_begin(), DTE = innerDstT->subtype_end(); DTI != DTE; ++DTI){
						if((llvm::Type*)DTI == innerSrcT){
							errs() << "Upcast: " << *II << " From: " << *srcT << " To: " << *dstT << "\n";
						}
						else
							errs() << "No Upcast" << *II << " From: " << *srcT << " To: " << *dstT << "\n";
						}
						*/

						if (isa<CallInst>(II->getOperand(0)))
						{
							...
							/*
							for (Type::subtype_iterator STI = srcT->subtype_begin(), STE = srcT->subtype_end(); STI != STE; ++STI){
								if((llvm::Type*)STI == dstT){
									errs() << "Downcast: " << *II << "From: " << *srcT << "To: " << *dstT << "\n";
								}
							}
							for (Type::subtype_iterator DTI = dstT->subtype_begin(), DTE = dstT->subtype_end(); DTI != DTE; ++DTI){
								if((llvm::Type*)DTI == srcT){
									errs() << "Upcast: " << *II << "From: " << *srcT << "To: " << *dstT << "\n";
								}
							}
							*/
		...
		void printStats()
		{
            ...
            // new added
            errs() << "##### The number of records of SEQ/DYN pointers : " << records.size() << "\n";
            record_filename = "recordings.txt";
            if (record_filename.size() > 0) {
                errs() << "##### Record file name : " << record_filename.substr(7) << "\n";
                //std::ofstream out(record_filename.substr(7)+".txt");
                std::ofstream out(record_filename);
                for (auto r : records){
                    out << r;
                    out << "\n";
                }
            ...
            //errs() << "##### Record file name : " << record_filename << "\n";
		...
		bool runOnModule(Module &M) override
		{
			...
			//ptaw.setupPTA(M);
            //
            // add for backup a unmodified Module
            errs() << "Module information before instrumentation\n";
            /*
            ...
            */
            ...
            SMDiagnostic Err;
            static IRBuilder<> Builder1(Context);
            Module *orgM = parseIRFile("./target-test.bc", Err, Context).release();
            errs() << " orgM name : " << orgM->getName() << "\n";
            for (auto i = orgM->begin(), e = orgM->end(); i != e; ++i){
                Function *F = &*i;
                ...
                for (inst_iterator ii = inst_begin(*F), e = inst_end(*F); ii != e; ++ii){
                    Instruction * I = &*ii;
                    //errs() << " " << *I << "\n";
                    //insert the info to maps
                    if (LoadInst *II = dyn_cast_or_null<LoadInst>(I)){
                        //errs() << "load inst: " << *II << "\n";
                        std::string key = func_name + std::to_string(index);
                        std::string str;
                        llvm::raw_string_ostream ss(str);
                        ss << *II;
                        std::string value = ss.str();
                        //errs() << "key : " << key << "\t value : " << value << "\n";
                        instOrigMap.insert(std::pair<std::string, std::string>(key, value));
                        index ++;
                    }
                    if (StoreInst *II = dyn_cast_or_null<StoreInst>(I)){
                        //errs() << "load inst: " << *II << "\n";
                        std::string key = func_name + std::to_string(index);
                        std::string str;
                        llvm::raw_string_ostream ss(str);
            ...
            //errs() << "Size of instOrigMap = " << instOrigMap.size() << "\n";
            */
			CurrentModule = &M;
			...
			//Classification Propagation Algorithm
			std::set<StringRef> functionsToAnalyze;
			Function *F = NULL;

			//Begin classification propagation for Globals whose classification changed
			errs() << "\n\n\n\n*********** Reclassification Propagation **************\n\n";

			//Compute the functions that need to be analyzed
			for (auto it = TheState.Variables.begin(); it != TheState.Variables.end(); it++)
			{
				//Only analyze dependent functions of Global whose classification changed
				if (it->second.isGlobal && it->second.didClassificationChange)
				{
					//Analyze and cache dependent functions
					...
					//Reverse the classification change state to unchanged
			...
			*/
			...
			identifyDifferentKindsOfUnsafePointers();
			//getFunctionCallsAndParams(&M);
		...
		void identifyDifferentKindsOfUnsafePointers() {
			...
			int 			noOperands = -1;
			int 			i = 1;
			int 			unsafeStackPointerCount = 0;
			int 			stackSeqPointerCount = 0;
			int 			stackDynPointerCount = 0;
			bool 			isUnsafeStackPointer = false;
			bool 			mayPointOutsideStack = false;
			const 			TargetLibraryInfo *TLI = NULL;
			std::set<Value *>unsafeStackPointerSet;
			std::set<Value *>stackSeqPointerSet;
			std::set<Value *>stackDynPointerSet;
			...
			for (std::map<const llvm::Value *, NesCheck::VariableInfo>::iterator it = TheState.Variables.begin();
			...
			errs() << NORMAL << "\n";
			errs() << BLUE << "Total Unsafe Stack Pointer Number by CCured: " << RED << unsafeStackPointerCount << NORMAL << "\n";
			errs() << BLUE << "Total Stack Seq Pointer Number: " << RED << stackSeqPointerCount << NORMAL << "\n";
			errs() << BLUE << "Total Stack Dyn Pointer Number: " << RED << stackDynPointerCount << NORMAL << "\n";
			errs() << NORMAL << "\n";
		}
	};
}
